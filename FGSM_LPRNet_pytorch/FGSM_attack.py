...
def get_parser():
    ...
    parser.add_argument('--test_batch_size', default=2, type=int, help='testing batch size.')
    ...
    parser.add_argument('--epsilon', default=0.1, type=float, help='the degree of disturbance(0~1)')
...
def Greedy_Decode_Eval(Net, datasets, args):
    ...
    for i in range(epoch_size):
        ...
        for i in range(len(preb_labels_ori)):   # ori和atk的长度应该是一样的
            ...
            atk_reg_correct += show(imgs[i], label_ori, label_atk, perturbed_images[i], show_img=args.show)
    
    # 显示攻击和识别信息
    reg_acc = (total - reg_wrong) * 1.0 / total * 100
    print("\nRecognition accuracy: {:d} / {:d} = {:.2f}%".format(total - reg_wrong, total, reg_acc))
    if total - reg_wrong > 0:
...
def fgsm_attack(image, epsilon, data_grad):
    ...
    sign_data_grad = data_grad.sign()   # 我们只为了得知扰动的方向，具体扰动的值由epsilon来确定。用sign来方向，注意是对每个像素都有一个方向
