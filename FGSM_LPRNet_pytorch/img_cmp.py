...
def draw_all_images(epsilons, examples):
    """
    画出所有图像
    """
    # 画出不同epsilon下图像之间的直观差异对比
    plt.figure(figsize=(5,4))
    for i in range(len(epsilons)):
        plt.subplot(5, 4, i + 1)
        plt.title("Eps:{}".format(epsilons[i]))
        plt.xticks([], [])
        plt.yticks([], [])
        plt.imshow(examples[i])
    plt.tight_layout()
    plt.savefig('./plot_figures/compare.jpg')
    ...
    for i in range(len(epsilons)):
        ...
        cosin.append(image_similarity_vectors_via_numpy(examples[0], examples[i]))

    # 画出ssim图像
    draw_similarity_image(epsilons, ssim, "SSIM", save_path='./plot_figures/ssim.jpg')
    # 画出psnr图像
    draw_similarity_image(epsilons, psnr, "PSNR", ylabel="dB", save_path='./plot_figures/psnr.jpg')
    # 画出余弦相似度图像
    draw_similarity_image(epsilons, cosin, "COSIN", save_path='./plot_figures/cosin.jpg',)

def draw_similarity_image(epsilons, data, title, ylabel=None, xticks=(0, 0.1, 0.005), yticks=None, save_path=None):
    '''
    画出相似度的图像
    '''
...
def image_similarity_vectors_via_numpy(image1, image2):
    ...
    for image in images:
        vector = []
        for pixel_tuple in image.getdata(): # 函数的作用是把图片的像素信息flatten到一维
