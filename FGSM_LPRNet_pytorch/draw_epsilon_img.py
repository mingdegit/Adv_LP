...
def test():
    ...
    try:
        for epsilon in epsilons:
            ...
            accuracies.append(Greedy_Decode_Eval(lprnet, batch_iterator, args, epsilon, epoch_size))
    finally:
    ...
    plt.xticks(np.arange(0, 0.075, step=0.005))
...
def Greedy_Decode_Eval(Net, batch_iterator, args, epsilon, epoch_size):
    ...
    for i in range(epoch_size):
        ...
        for i in range(len(preb_labels_ori)):   # ori和atk的长度应该是一样的
            ...
            # 若预测的长度和正确的长度不相等 或 两者的值不一样，则表示原来就识别错误
            ...
            if label_ori == label_atk:
                atk_reg_correct += 1
    
    # 显示攻击和识别信息
    atk_reg_acc = atk_reg_correct * 1.0 / total
    print("Epsilon: {}\tRecognition accuracy: {:d} / {:d} = {:.4f}".format(epsilon, atk_reg_correct, total, atk_reg_acc))
    return atk_reg_acc
